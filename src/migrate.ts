...
async function showHistory() {
    ...
    if(totalAvailableUpMigrations > 0) {
        console.log(`Total available "up" migrations: ${totalAvailableUpMigrations}`);
    } else {
...
if (require.main === module) {
    ...
    if(!validatedArgs.success) {
        console.error('Invalid argument. `tsx src/db-migrate.ts <history|up|upToLatest|down>`');
        process.exit(1);
    }
